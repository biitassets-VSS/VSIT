...
import { LayoutDashboard, Laptop, Ticket, ClipboardCheck, LogOut, Menu, X, ChevronDown, Loader2 } from 'lucide-react';
import { supabase } from '@/lib/supabaseClient';

interface StaffUser { name: string; empCode: string; initials: string; email: string; }
...
export default function StaffLayout({ children }: { children: React.ReactNode }) {
  ...
  const [staffUser, setStaffUser] = useState<StaffUser>({ name: '...', empCode: '...', initials: 'SM', email: '' });
  ...
  useEffect(() => {
    const verifyStaff = async () => {
      ...
      setStaffUser({
        ...
        initials: (userProfile.name || 'SM').substring(0, 2).toUpperCase(),
        email: user.email || ''
      });
