...
import { Users, Monitor, ArrowLeft, Loader2, ShieldAlert } from 'lucide-react';
...
export default function AdminRemotePage() {
  ...
  const loadStaffData = async () => {
    try {
      // 1. Verify Admin Auth (Using your existing pattern)
      ...
      // 2. Fetch all profiles to populate the connection list
      ...
      <div className="max-w-[1600px] mx-auto space-y-6 h-[calc(100vh-4rem)] flex flex-col">
        ...
        <div className="flex flex-col lg:flex-row gap-6 flex-1 min-h-0">
          ...
          <div className="w-full lg:w-80 bg-white rounded-3xl border border-slate-200 shadow-sm flex flex-col shrink-0 overflow-hidden">
            <div className="p-5 border-b border-slate-100 bg-slate-50/50">
              <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                <Users size={14} /> Available Staff ({staffList.length})
              </h3>
            </div>
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
              {staffList.map((staff) => (
                ...
                >
                  ...
                  <p className="text-xs font-semibold text-slate-400 mt-1">ID: {staff.emp_code || 'N/A'}</p>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 rounded-3xl border border-slate-200 shadow-sm overflow-hidden bg-white relative flex flex-col">
            {activeSession ? (
              // Using emp_code or id as the target ID for RustDesk connection
              <RemoteDesktopViewer 
