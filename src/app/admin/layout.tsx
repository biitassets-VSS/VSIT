...
export default function StaffLayout({ children }: { children: React.ReactNode }) {
  ...
  // 🌟 FIX: Instant Logout (No 'await', wipe memory first, force redirect immediately)
  const handleLogout = () => {
    localStorage.clear();
        ...
        <header className="h-20 bg-white border-b border-slate-100 shrink-0 flex items-center justify-between px-4 lg:px-8 z-30">
          ...
          <div className="relative">
            <button 
              ...
              className="relative p-2.5 rounded-xl bg-slate-50 text-slate-600 hover:bg-slate-100 transition-colors border border-slate-100 shadow-sm cursor-pointer"
            >
              <Bell size={20} />
                      ...
                      {notifications.map((notif) => (
                        <div key={notif.id} className="p-4 hover:bg-slate-50 transition-colors group">
                          ...
                          <button 
                            ...
                            className="text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 text-slate-400 hover:text-emerald-600 transition-colors cursor-pointer"
                          >
