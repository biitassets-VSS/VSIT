...
const SearchableStaffDropdown = ({ value, onChange, staffList, isDarkMode, placeholder = "Search name or emp code..." }: any) => {
  ...
  const t = {
    bg: isDarkMode ? 'bg-zinc-950 border-zinc-800' : 'bg-white border-slate-200',
    text: isDarkMode ? 'text-zinc-100' : 'text-slate-900',
    subText: isDarkMode ? 'text-zinc-400' : 'text-slate-500',
    menu: isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200',
    hover: isDarkMode ? 'hover:bg-zinc-800' : 'hover:bg-blue-50',
    header: isDarkMode ? 'bg-zinc-950 border-zinc-800 text-zinc-500' : 'bg-slate-50 border-slate-100 text-slate-500',
  };
      ...
      {open && (
        <div className={`absolute z-50 w-full mt-2 border rounded-xl shadow-2xl max-h-60 overflow-y-auto custom-scrollbar ${t.menu}`}>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((s: any) => (
              <div 
                key={s.id} className={`p-3.5 text-xs cursor-pointer border-b ${isDarkMode ? 'border-zinc-800/50' : 'border-slate-50'} flex justify-between items-center transition-colors group ${t.hover}`}
              ...
              >
                ...
                <span className={`font-mono text-[10px] px-2 py-0.5 rounded-md transition-colors ${isDarkMode ? 'bg-zinc-800 text-zinc-400 group-hover:bg-blue-500/20 group-hover:text-blue-400' : 'bg-slate-100 text-slate-500 group-hover:bg-blue-100 group-hover:text-blue-600'}`}>
                  {s.emp_code || s.email}
                </span>
              </div>
            ))
...
export default function AssetRegistryPage() {
  ...
  const getInspectionStatusColor = (status: string) => {
    ...
    return isDarkMode ? 'text-zinc-400 bg-zinc-800 border-zinc-700' : 'text-slate-600 bg-slate-50 border-slate-200';
  };
  ...
  // ... Bulk upload and CSV functions stay exactly the same logically ...
  const downloadSampleCsvTemplate = () => { /* ... existing code ... */ };
  const executeBulkImport = async () => { /* ... existing code ... */ };
  ...
  // 🖨️ UPDATED: Clean, High-Readability Print Sticker Logic
  ...
  const theme = {
    ...
    card: isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200/60',
    textMain: isDarkMode ? 'text-zinc-100' : 'text-slate-800',
    textSub: isDarkMode ? 'text-zinc-400' : 'text-slate-500',
    inputBg: isDarkMode ? 'bg-zinc-950 border-zinc-800 focus:border-blue-500 text-zinc-100 placeholder-zinc-500' : 'bg-slate-50 border-slate-200 focus:border-blue-500 text-slate-900 placeholder-slate-400',
    cardHover: isDarkMode ? 'hover:border-zinc-700 hover:bg-zinc-800/50' : 'hover:border-blue-300 hover:shadow-md',
    modalOverlay: 'bg-zinc-950/80 backdrop-blur-sm',
    modalBody: isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200',
    modalHeader: isDarkMode ? 'bg-zinc-950 border-zinc-800' : 'bg-slate-50 border-slate-100',
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <span className={`px-3 py-1 rounded-full text-[10px] font-semibold uppercase tracking-widest ${isDarkMode ? 'bg-zinc-800 text-zinc-300' : 'bg-slate-100 text-slate-700'}`}>{assets.length} Units</span>
              </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 custom-scrollbar">
            {[
              ...
              >
                ...
                <span className={`px-2 py-0.5 rounded-md font-mono text-[10px] ${selectedCategory === cat.name ? 'bg-white/20 text-white' : isDarkMode ? 'bg-zinc-800 text-zinc-400' : 'bg-slate-100 text-slate-500'}`}>{getCatCount(cat.name)}</span>
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {filteredAssets.map(asset => (
              <div key={asset.id} className={`${theme.card} rounded-3xl border shadow-sm flex flex-col justify-between group transition-all ${theme.cardHover} overflow-hidden`}>
                ...
                <div className={`p-5 border-b ${isDarkMode ? 'border-zinc-800' : 'border-slate-50'}`}>
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <button onClick={() => openAssetViewModal(asset)} className={`p-2.5 rounded-xl transition-colors cursor-pointer border ${isDarkMode ? 'bg-zinc-800 border-zinc-700 text-zinc-400 hover:bg-zinc-700 hover:text-white' : 'bg-slate-50 border-slate-100 text-slate-400 hover:bg-blue-50 hover:text-blue-600'}`}>
                      <QrCode size={18} />
                    </button>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`px-2.5 py-1 rounded-md font-bold text-[9px] uppercase tracking-widest border ${isDarkMode ? 'bg-zinc-800 text-zinc-300 border-zinc-700' : 'bg-slate-50 text-slate-600 border-slate-200'}`}>{asset.asset_condition || 'New'}</span>
                  </div>
                </div>

                <div className={`p-5 space-y-3 flex-1 ${isDarkMode ? 'bg-zinc-900/50' : 'bg-slate-50/50'}`}>
                  ...
                  <div className={`flex justify-between items-center p-3 rounded-xl border shadow-sm transition-colors ${theme.card} group-hover:border-blue-500/30`}>
                    ...
                    <span className={`font-mono font-bold px-2 py-1 rounded-lg text-[9px] ${isDarkMode ? 'bg-zinc-800 text-zinc-400' : 'bg-slate-100 text-slate-500'}`}>{asset.emp_code}</span>
                  </div>
                </div>

                <div className={`p-4 border-t flex items-center justify-between ${isDarkMode ? 'bg-zinc-950/50 border-zinc-800' : 'bg-slate-100/80 border-slate-200'}`}>
                  <div className="flex items-center gap-2">
        ...
        return (
          <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 ${theme.modalOverlay}`}>
            <div className={`rounded-3xl max-w-5xl w-full max-h-[90vh] overflow-hidden shadow-2xl flex flex-col md:flex-row border ${theme.modalBody}`}>
              ...
              <div className={`w-full md:w-[35%] p-8 flex flex-col items-center border-b md:border-b-0 md:border-r ${isDarkMode ? 'bg-zinc-950/50 border-zinc-800' : 'bg-slate-50 border-slate-200'} relative shrink-0`}>
                <button onClick={() => setViewAssetModal(null)} className={`absolute md:hidden top-4 right-4 p-2 rounded-full shadow-sm ${isDarkMode ? 'bg-zinc-800 text-zinc-400' : 'bg-white text-slate-400'}`}><X size={14}/></button>
                ...
                {/* 3. Asset ID (No black button, highly readable) */}
              ...
              <div className={`w-full md:w-[65%] flex flex-col overflow-y-auto custom-scrollbar relative ${theme.modalBody}`}>
                <button onClick={() => setViewAssetModal(null)} className={`hidden md:flex absolute top-6 right-6 p-2.5 rounded-full cursor-pointer z-10 transition-colors ${isDarkMode ? 'bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-white' : 'bg-slate-100 hover:bg-slate-200 text-slate-400 hover:text-slate-900'}`}><X size={18}/></button>

                <div className="p-8 md:p-10 space-y-8">
                  
                  <div className={`flex flex-col sm:flex-row sm:items-center justify-between pb-6 border-b gap-4 ${isDarkMode ? 'border-zinc-800' : 'border-slate-100'}`}>
                    <div className="flex items-center gap-3">
                  ...
                  {isEditingAsset ? (
                    <div className="space-y-6 animate-in fade-in duration-200">
                      ...
                      <div className={`flex justify-between items-center pb-3 border-b ${isDarkMode ? 'border-zinc-800' : 'border-blue-100'}`}>
                      ...
                      <div className={`grid grid-cols-1 sm:grid-cols-2 gap-5 p-5 rounded-2xl border ${isDarkMode ? 'bg-zinc-950/50 border-zinc-800' : 'bg-blue-50/30 border-blue-100'}`}>
                        <div>
                        ...
                        <div>
                          ...
                          <input type="text" value={editForm.asset_tag} onChange={e => setEditForm({...editForm, asset_tag: e.target.value})} className={`w-full p-3.5 rounded-xl text-xs font-mono font-bold outline-none uppercase transition-colors border ${isDarkMode ? 'bg-zinc-950 border-blue-500/50 text-blue-400 focus:border-blue-400' : 'bg-white border-blue-300 text-blue-900 focus:border-blue-500'}`} />
                        </div>
                      </div>
                      ...
                      {/* Other inputs converted to theme logic securely... */}
                      ...
                      <div className={`grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t ${isDarkMode ? 'border-zinc-800' : 'border-slate-100'}`}>
                      ...
                      <div className={`p-5 rounded-2xl border ${isDarkMode ? 'bg-zinc-950 border-zinc-800' : 'bg-slate-50 border-slate-200'}`}>
                      ...
                      <div className="flex gap-4 pt-6">
                        <button type="button" onClick={() => setIsEditingAsset(false)} className={`px-8 py-4 rounded-xl text-xs font-bold uppercase tracking-wider cursor-pointer transition-colors border ${isDarkMode ? 'bg-zinc-900 border-zinc-800 text-zinc-300 hover:bg-zinc-800' : 'bg-white border-slate-200 hover:bg-slate-50 text-slate-700'}`}>Cancel</button>
                    ...
                    <div className="space-y-6">
                      ...
                      <div className={`p-5 rounded-2xl border flex items-center justify-between ${isDarkMode ? 'bg-blue-500/5 border-blue-500/20' : 'bg-blue-50/50 border-blue-100'}`}>
                        ...
                        <div className="flex flex-col items-end">
                           ...
                           <span className={`text-sm font-mono font-bold px-3 py-1 rounded-lg border shadow-sm ${isDarkMode ? 'bg-zinc-900 border-blue-500/30 text-blue-400' : 'bg-white border-blue-100 text-blue-800'}`}>{viewAssetModal.emp_code}</span>
                        </div>
                      </div>
                    </div>
                  )}
