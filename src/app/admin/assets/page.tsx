...
import {
  ...
  DollarSign, Wrench, Hash, Trash2, UserMinus, X, Pencil, Clock
} from 'lucide-react';
...
interface Asset {
  ...
  notes?: string;
  photos?: string[];
  inspection_status?: 'Passed' | 'Failed' | 'Pending Repair' | 'Pending';
...
const CATEGORY_PREFIX_MAP: Record<string, string> = {
  'Laptop': 'LAP', 'Monitor': 'MON', 'Mouse': 'MOU', 'Headphone': 'HDP',
  'Keyboard': 'KBD', 'Wired Keyboard Combo': 'WKC', 'Wireless Keyboard Combo': 'WMC',
  'Stand': 'STN', 'Cleaning Kit': 'CLN', 'Mobile Phone': 'MOB', 'Other': 'OTH'
};
    ...
    const fetchDatabaseRecords = async () => {
      try {
        ...
        if (assetsResponse.data) {
          const mappedAssets: Asset[] = assetsResponse.data.map((dbAsset: any) => ({
            id: dbAsset.id,
            ...
            last_inspection_date: dbAsset.last_inspection_date || '-',
            next_inspection_date: dbAsset.next_inspection_date || '-',
            inspection_notes: dbAsset.inspection_notes || 'No inspection notes available.'
          }));
  ...
  const handleEditClick = () => {
    if (selectedAsset) {
      setSingleAssetForm({
        ...
        status: selectedAsset.status as any,
              ...
              <div className="space-y-8">
                ...
                <div>
                   ...
                   <div className="bg-white rounded-2xl p-5 border border-gray-200 shadow-sm relative overflow-hidden">
                      {isOverdue(selectedAsset.next_inspection_date) && (
                        <div className="absolute top-0 right-0 bg-red-500 text-white text-[10px] font-black px-3 py-1 rounded-bl-lg flex items-center gap-1">
                          <AlertCircle size={10}/> OVERDUE
                        </div>
                      )}
                      <div className="flex items-center justify-between mb-4 mt-2">
                        <div>
                           <p className="text-xs font-bold text-gray-500 mb-1">Current Status</p>
                           <span className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider ${
                             selectedAsset.inspection_status === 'Passed' ? 'bg-green-100 text-green-700' :
                             selectedAsset.inspection_status === 'Failed' ? 'bg-red-100 text-red-700' :
                             selectedAsset.inspection_status === 'Pending Repair' ? 'bg-orange-100 text-orange-700' :
                        ...
                        <div className="text-right">
                           <p className="text-xs font-bold text-gray-500 mb-1">Last Inspected</p>
                           <p className="font-black text-gray-900 text-sm flex items-center gap-1 justify-end"><Clock size={14} className="text-gray-400"/> {selectedAsset.last_inspection_date}</p>
                        </div>
                      </div>
                      
                      <div className="bg-gray-50 p-3 rounded-xl border border-gray-100 mt-2">
                         <p className="text-xs font-bold text-gray-500 mb-1">Inspection Notes</p>
                         <p className="text-sm font-medium text-gray-800">{selectedAsset.inspection_notes}</p>
                      </div>
                   </div>
                </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="max-h-60 overflow-y-auto space-y-2 pr-1">
                {filteredStaff.length === 0 ? (
                  <p className="text-center text-sm font-bold text-gray-400 py-4">No staff found in database.</p>
                ) : (
                  filteredStaff.map(staff => (
                    <div key={staff.emp_code} onClick={() => updateAssetStatus('Assigned', { empCode: staff.emp_code, name: staff.name })} className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 hover:border-[#008b74] hover:bg-[#e6f4f1] cursor-pointer transition">
                      <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center font-bold text-gray-600">
                        {staff.name ? staff.name.charAt(0) : '?'}
                      </div>
                      <div>
                        <div className="text-sm font-black text-gray-900">{staff.name}</div>
                        <div className="text-xs font-bold text-gray-500">{staff.emp_code}</div>
                      </div>
                    </div>
                  ))
                )}
