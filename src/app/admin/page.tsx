                  ...
                  {recentActivity.map((log: any, i: number) => (
                    <div key={i} className={`flex gap-3 relative pb-4 border-b last:border-0 last:pb-0 shrink-0 ${isDarkMode ? 'border-white/10' : 'border-white/40'}`}>
                      ...
                      <div className="pt-0.5 min-w-0 flex-1">
                        ...
                        <p className={`text-[11px] font-medium mt-1 truncate ${theme.textSub}`}>{log.actionText}</p>
      ...
      {isBroadcastModalOpen && (
        <div className={`fixed inset-0 z-[100] flex flex-col items-center justify-start pt-24 sm:pt-28 pb-6 px-4 backdrop-blur-sm animate-in fade-in duration-200 ${isDarkMode ? 'bg-black/60' : 'bg-black/20'}`}>
          <div className={`relative max-w-lg w-full flex flex-col overflow-hidden shadow-[0_32px_80px_rgba(0,0,0,0.15)] flex-1 max-h-full rounded-[2rem] animate-in zoom-in-95 duration-200 ${theme.glassCard}`}>
            ...
            <form onSubmit={handleSendBroadcast} className="flex-1 overflow-y-auto custom-scrollbar flex flex-col p-4 sm:p-5 space-y-5">
              <div>
                <label className={`text-[10px] font-bold uppercase tracking-widest block mb-1.5 ${theme.textSub}`}>Message Text *</label>
                <textarea 
                  rows={4} 
                  required 
                  ...
                  className={`w-full p-4 rounded-xl outline-none text-[13px] font-medium transition-all resize-none ${theme.inputBg}`} 
                />
              </div>
              <div>
                <label className={`text-[10px] font-bold uppercase tracking-widest block mb-1.5 ${theme.textSub}`}>Attach Graphic / Flyer (Optional)</label>
                <label className={`cursor-pointer w-full p-5 rounded-xl border-2 border-dashed flex flex-col items-center justify-center gap-2.5 transition-all ${theme.glassInnerCard} hover:border-orange-500/50`}>
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-wider text-center">{broadcastImage ? `Attached: ${broadcastImage.name}` : 'Click to browse image file'}</span>
              ...
              <div className={`pt-5 mt-auto border-t flex gap-4 shrink-0 ${isDarkMode ? 'border-white/10' : 'border-white/40'}`}>
                <button type="button" onClick={() => setIsBroadcastModalOpen(false)} className={`px-6 py-3 rounded-xl ${theme.glassInnerCard} ${theme.textMain} hover:bg-white/60 transition-all text-[11px] font-bold uppercase tracking-widest cursor-pointer shadow-sm active:scale-95`}>
                  Cancel
                </button>
