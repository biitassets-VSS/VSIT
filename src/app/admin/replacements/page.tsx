...
export default function AdminReplacementsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const compiledPending = (pendingAssets || []).map(asset => ({
        ...asset, user_profile: pendProfiles.find(p => p.id === asset.assigned_to) || null
      }));
      setPendingRequests(compiledPending);

      const { data: availableAssets } = await supabase.from('assets').select('id, name, serial_number, asset_tag, category').eq('status', 'In Stock (Unassigned)');
      setAvailableInventory(availableAssets || []);

      const { data: historyInsps } = await supabase.from('inspections').select('*, assets(*)').in('status', ['Replacement Approved', 'Replacement Rejected']).order('created_at', { ascending: false });
      ...
      const compiledHistory = (historyInsps || []).map(insp => ({
        ...insp, user_profile: histProfiles.find(p => p.id === insp.inspected_by) || null
      }));
  ...
  const handleApproveSwap = async (oldAsset: any) => {
    ...
    if (!window.confirm("Confirm replacement?")) return;
    setProcessingId(oldAsset.id);
    const newAsset = availableInventory.find(a => a.id === selectedReplacementId);
    try {
      await supabase.from('assets').update({ assigned_to: null, status: 'In Repair' }).eq('id', oldAsset.id);
      await supabase.from('assets').update({ assigned_to: oldAsset.assigned_to, status: 'Assigned' }).eq('id', selectedReplacementId);
      await supabase.from('inspections').insert({
        asset_id: oldAsset.id, inspected_by: oldAsset.assigned_to, status: 'Replacement Approved', 
        notes: `[ADMIN SWAP] Old S/N: ${oldAsset.serial_number} replaced with New S/N: ${newAsset.serial_number}`
      });
      await supabase.from('notifications').insert({
        target_role: oldAsset.assigned_to, title: 'Hardware Replaced',
        message: `Your replacement request has been approved. Assigned: ${newAsset.name} (S/N: ${newAsset.serial_number}).`,
        is_read: false
      });
      setModal({ isOpen: false, data: null });
      setSelectedReplacementId('');
      toast.success('Asset replaced & assigned.');
      fetchData(); 
    } catch (error: any) {
  ...
  const handleRejectReplacement = async (oldAsset: any) => {
    ...
    try {
      ...
      await supabase.from('inspections').insert({
        asset_id: oldAsset.id, inspected_by: oldAsset.assigned_to, status: 'Replacement Rejected', notes: `Admin Rejected: ${reason}`
      });
      await supabase.from('notifications').insert({
        target_role: oldAsset.assigned_to, title: 'Replacement Request Denied',
        message: `Request to replace ${oldAsset.name} denied. Reason: ${reason}`, is_read: false
      });
      setModal({ isOpen: false, data: null });
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-900 flex items-center gap-3">
                <ArrowRightLeft className="text-orange-600" /> Hardware Replacements
              </h1>
            ...
            {pendingRequests.length === 0 ? (
              ...
              <div className="grid grid-cols-1 gap-4">
                {pendingRequests.map((request, index) => {
                  ...
                  return (
                    <div key={`pending-${request.id}-${index}`} className="bg-white p-6 rounded-2xl border border-orange-200/50 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-sm hover:shadow-lg hover:-translate-y-1 hover:border-orange-300 transition-all duration-300">
                      <div>
                        <div className="flex items-center gap-3 mb-1">
                          <span className="px-2 py-0.5 bg-orange-100 text-orange-700 text-[10px] font-black uppercase rounded-md">Swap Requested</span>
                      ...
                      <div className="w-full md:w-auto">
                        <button onClick={() => openSwapModal(request)} className="w-full md:w-auto px-6 py-3 bg-orange-600 hover:bg-orange-700 text-white rounded-xl text-sm font-bold flex items-center justify-center gap-2 shadow-sm transition-all hover:scale-105 active:scale-95">
            ...
            {historyRequests.length === 0 ? (
              ...
              <div className="grid grid-cols-1 gap-4">
                {historyRequests.map((record, index) => {
                  ...
                  return (
                    <div key={`history-${record.id}-${index}`} className="bg-white p-5 rounded-2xl border border-slate-200 flex flex-col shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 opacity-90">
      ...
      {modal.isOpen && modal.data && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-xs z-[9999] flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden border border-slate-100 flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300">
            <div className="p-6 bg-purple-50 border-b border-purple-100 flex items-center justify-between shrink-0">
              <h3 className="font-extrabold text-purple-900 text-sm tracking-tight uppercase flex items-center gap-2">
                <ArrowRightLeft size={16} className="text-purple-600"/> Execute Device Swap
              </h3>
              <button onClick={() => setModal({ isOpen: false, data: null })} className="p-2 rounded-full hover:bg-white/50 text-purple-500 transition-all hover:scale-110"><XCircle size={20}/></button>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              <div className="p-5 border-2 border-dashed border-orange-200 bg-orange-50/50 rounded-2xl relative">
                ...
                <p className="text-sm font-semibold text-slate-600 mt-3 border-t border-orange-200/50 pt-3">
              ...
              <div className="p-5 border-2 border-dashed border-purple-200 bg-purple-50/50 rounded-2xl relative">
                <div className="absolute -top-3 left-4 bg-white px-2 text-xs font-black text-purple-500 uppercase tracking-widest">Select Replacement (In Stock)</div>
                ...
                <div className="mt-4">
                  ...
                  <select 
                    ...
                    className="w-full p-4 rounded-xl border border-purple-200 bg-white outline-none focus:ring-2 focus:ring-purple-500/20 text-sm font-semibold text-slate-900 transition-all"
                  >
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-200 flex justify-between gap-3 shrink-0">
              <button onClick={() => handleRejectReplacement(modal.data)} className="px-6 py-3 rounded-xl text-xs font-bold text-rose-600 bg-rose-50 border border-rose-200 hover:bg-rose-100 transition-all hover:scale-105 active:scale-95">
                Reject Swap Request
              </button>
              <button disabled={!selectedReplacementId || processingId === modal.data.id} onClick={() => handleApproveSwap(modal.data)} className="px-8 py-3 rounded-xl text-xs font-bold text-white bg-orange-600 hover:bg-orange-700 shadow-sm transition-all hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
