...
export default function StaffLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const verifyStaff = async () => {
      ...
      setStaffProfile({
        id: user.id, // Set UUID for fetching targeted notifications
      ...
      // Fetch Staff Notifications based on their specific User ID
      fetchNotifications(user.id);

      // REAL-TIME LISTENER FOR STAFF ALERTS
      const channel = supabase
        ...
        .on('postgres_changes', { 
          ...
          schema: 'public', 
          table: 'notifications',
          filter: `target_user=eq.${user.id}` // Only listen for notifications meant for THIS user
        }, (payload) => {
              ...
              {notifications.length === 0 ? (
                ...
                notifications.map(n => (
                  <div key={n.id} onClick={() => markAsRead(n.id)} className={`p-3 rounded-xl cursor-pointer transition-colors mb-1 ${n.is_read ? 'bg-white opacity-60' : 'bg-orange-50/50 border border-orange-100'}`}>
                    <p className="text-[11px] font-bold text-[#ff9800] mb-0.5 uppercase tracking-wider">{n.type}</p>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        <header className="lg:hidden h-16 bg-white border-b border-gray-100 flex items-center justify-between px-4 shadow-sm shrink-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 text-gray-500 hover:bg-orange-50 hover:text-[#ff9800] rounded-lg"><Menu size={24} /></button>
          </div>
        </header>
