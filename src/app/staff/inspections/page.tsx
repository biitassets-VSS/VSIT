...
export default function StaffInspectionsPage() {
  ...
  return (
    <>
      {/* 🌟 MAIN PAGE CONTENT (Unrestricted scrolling allowed) */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8 space-y-6 animate-in fade-in duration-500 w-full select-none" onContextMenu={(e) => e.preventDefault()}>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 mb-4">
                      <div className="min-w-0">
                        ...
                        <span className="font-bold text-xs sm:text-sm text-slate-900 break-words whitespace-normal block">{asset.asset_tag || 'N/A'}</span>
                      </div>
                      <div className="min-w-0">
                    ...
                    <div className={`p-3.5 rounded-xl border flex-1 backdrop-blur-xs shadow-inner mb-4 ${isRejected ? 'bg-rose-50/50 border-rose-200/50' : isApproved ? 'bg-emerald-50/50 border-emerald-200/50' : 'bg-slate-50/50 border-slate-200/50'}`}>
                      ...
                      <p className={`text-xs sm:text-sm font-semibold whitespace-pre-wrap break-words ${isRejected ? 'text-rose-900' : isApproved ? 'text-emerald-900' : 'text-slate-700'}`}>
      ...
      {/* 🌟 SECURE LIGHTBOX (Completely moved outside the page flow to prevent z-index/stacking context issues) */}
      {photoViewer.isOpen && (
        <div 
          className="fixed inset-0 bg-slate-950/98 backdrop-blur-3xl flex flex-col items-center justify-center p-0 m-0 select-none overflow-hidden"
