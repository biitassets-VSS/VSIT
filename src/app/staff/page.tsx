...
export default function StaffDashboardPage() {
  ...
  const loadRealDatabase = async () => {
    try {
      ...
      // FIX: The tickets fetch now looks for BOTH user.id and user.emp_id so tickets never disappear!
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-8 font-sans text-slate-800">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm flex flex-col md:flex-row justify-between md:items-center gap-4">
          ...
          <button onClick={() => loadRealDatabase()} className="flex items-center gap-2 px-4 py-3 bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors border border-slate-200">
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
          ...
          <div className="flex overflow-x-auto border-b border-slate-100 bg-slate-50/50 custom-scrollbar">
            {[
              ...
              <button 
                ...
                className={`flex items-center gap-2.5 px-6 py-5 text-xs font-black uppercase tracking-widest whitespace-nowrap transition-colors border-b-2 ${
              ...
              >
                ...
                <span className={`px-2 py-0.5 rounded-md text-[9px] ${activeTab === tab.id ? 'bg-blue-100 text-blue-800' : 'bg-slate-200 text-slate-600'}`}>{tab.count}</span>
              </button>
            ))}
          </div>
          ...
          {activeTab === 'assets' && (
            <div className="p-6 md:p-8 space-y-5 bg-white">
              {assignedAssets.length === 0 ? (
                <div className="py-16 text-center text-slate-400 font-bold text-xs">No hardware units currently assigned to you.</div>
              ) : (
                    ...
                    <div key={asset.id} className="bg-slate-50 rounded-2xl p-6 border border-slate-200 space-y-4 hover:border-blue-300 transition-colors">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <span className="text-xs font-mono font-bold text-slate-500">S/N: {asset.serial_number || 'N/A'}</span>
                            <span className="text-[10px] font-mono font-black text-blue-600 bg-blue-100 px-2 py-0.5 rounded-md">{asset.asset_tag}</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="px-3 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-lg text-[10px] font-black uppercase tracking-wider">{asset.status}</span>
                          {isOverdue && <span className="px-3 py-1 bg-rose-500 text-white rounded-lg text-[10px] font-black uppercase tracking-wider shadow-xs animate-pulse">ACTION REQUIRED</span>}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                        <div className="bg-white p-4 rounded-xl border border-slate-200 flex items-center gap-3 shadow-xs">
                          ...
                          <div><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Last Audited</p><p className="text-xs font-black text-slate-800 mt-0.5">{asset.last_inspection_date ? new Date(asset.last_inspection_date).toLocaleDateString('en-IN') : 'Pending Initial Setup'}</p></div>
                        </div>
                        <div className="bg-white p-4 rounded-xl border border-slate-200 flex items-center gap-3 shadow-xs">
                          <Calendar className="text-slate-400 shrink-0" size={18} />
                          <div><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Next Due Date</p><p className="text-xs font-black text-blue-700 mt-0.5">{calculateUpcomingDate(asset.last_inspection_date || asset.created_at)}</p></div>
                        </div>
                        <button onClick={() => setModal({ isOpen: true, type: 'INSPECTION', targetAsset: asset })} className="w-full bg-slate-900 hover:bg-slate-800 text-white font-black text-[10px] uppercase tracking-widest rounded-xl py-3 transition-colors flex items-center justify-center cursor-pointer shadow-md">
          ...
          {activeTab === 'inspections' && (
            <div className="p-6 md:p-8 bg-white space-y-4">
              {myPastInspections.length === 0 ? (
                <div className="py-16 text-center text-slate-400 font-bold text-xs">No past inspection logs found.</div>
              ) : (
                <div className="grid grid-cols-1 gap-4">
                  {myPastInspections.map((log: any) => {
                    const isApproved = log.status?.toLowerCase().includes('approved') || log.status?.toLowerCase().includes('pass');
                    const isRejected = log.status?.toLowerCase().includes('reject') || log.status?.toLowerCase().includes('not');

                    return (
                      <div key={log.id} className="p-5 rounded-2xl bg-slate-50 border border-slate-200 flex flex-col md:flex-row justify-between md:items-center gap-4 hover:shadow-sm transition-all">
                        <div className="space-y-1">
                          <h4 className="text-sm font-black text-slate-900">{log.assets?.asset_name || 'Hardware Unit'}</h4>
                          ...
                          <p className="text-[10px] italic text-slate-400 pt-1">"{log.notes || 'No description provided'}"</p>
                        </div>
                        
                        <div className="flex items-center gap-3 shrink-0">
                          <div className={`px-4 py-2 rounded-xl flex items-center gap-2 border ${isApproved ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : isRejected ? 'bg-rose-50 text-rose-700 border-rose-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
                            {isApproved ? <CheckCircle2 size={16}/> : isRejected ? <AlertTriangle size={16}/> : <Hourglass size={16}/>}
                            <span className="text-[10px] font-black uppercase tracking-widest">{log.status || 'Pending'}</span>
                          </div>
                        </div>
                      </div>
                    );
          ...
          {/* TAB: IT SUPPORT TICKETS */}
          {activeTab === 'tickets' && (
            <div className="p-6 md:p-8 bg-white space-y-4">
              <div className="flex justify-end mb-4">
                <button onClick={() => setModal({ isOpen: true, type: 'TICKET' })} className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-md cursor-pointer flex items-center gap-2">
                  <PlusCircle size={14}/> Raise New Ticket
                </button>
              </div>

              {myTickets.length === 0 ? (
                <div className="py-12 text-center text-slate-400 font-bold text-xs">You have no active or past IT support tickets.</div>
              ) : (
                <div className="space-y-4">
                  {myTickets.map(ticket => {
                    const isClosed = ticket.status?.toLowerCase() === 'closed' || ticket.status?.toLowerCase() === 'resolved';
                    return (
                      <div key={ticket.id} className="p-5 bg-white border border-slate-200 rounded-2xl shadow-sm hover:border-blue-300 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="space-y-1.5 flex-1">
                          <div className="flex items-center gap-2">
                            <span className="px-2 py-0.5 bg-slate-100 text-slate-500 rounded text-[9px] font-black uppercase tracking-widest">{ticket.category}</span>
                            <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest border ${isClosed ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-blue-50 text-blue-700 border-blue-200'}`}>
                              {ticket.status || 'Open'}
                            </span>
                          </div>
                          <h4 className="text-sm font-black text-slate-900">{ticket.title}</h4>
                        ...
                        <div className="flex items-center gap-4 bg-slate-50 p-3 rounded-xl border border-slate-100 shrink-0">
                          <div className="text-left">
                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Raised On</p>
                            <p className="text-[11px] font-bold text-slate-800">{new Date(ticket.created_at).toLocaleDateString()}</p>
                          </div>
                          <div className="w-px h-6 bg-slate-200"></div>
                          <div className="text-left min-w-[80px]">
                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Turnaround</p>
                            <p className={`text-[11px] font-black ${isClosed ? 'text-emerald-600' : 'text-amber-600'}`}>
          ...
          {activeTab === 'requests' && (
            <div className="p-6 md:p-8 bg-white space-y-4">
              <div className="flex justify-end mb-4">
                <button onClick={() => setModal({ isOpen: true, type: 'REQUEST' })} className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-md cursor-pointer flex items-center gap-2">
                  <PlusCircle size={14}/> Request Hardware
                </button>
              </div>

              {myRequests.length === 0 ? (
                <div className="py-12 text-center text-slate-400 font-bold text-xs">No hardware requests submitted.</div>
              ) : (
                <div className="space-y-4">
                  {myRequests.map(req => {
                    const isClosed = req.status?.toLowerCase() === 'closed' || req.status?.toLowerCase() === 'approved';
                    return (
                      <div key={req.id} className="p-5 bg-emerald-50/30 border border-emerald-100 rounded-2xl shadow-sm hover:border-emerald-300 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="space-y-1.5 flex-1">
                          <div className="flex items-center gap-2">
                            <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded text-[9px] font-black uppercase tracking-widest border border-emerald-200">{req.category} Request</span>
                            <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest border ${isClosed ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
                              {req.status || 'Pending Approval'}
                            </span>
                          </div>
                          <h4 className="text-sm font-black text-slate-900">{req.title}</h4>
                          <p className="text-[11px] text-slate-600 line-clamp-1">Justification: {req.description}</p>
                        </div>
                        
                        <div className="flex items-center gap-4 bg-white p-3 rounded-xl border border-slate-200 shrink-0">
                          <div className="text-left">
                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Requested</p>
                            <p className="text-[11px] font-bold text-slate-800">{new Date(req.created_at).toLocaleDateString()}</p>
                          </div>
                          <div className="w-px h-6 bg-slate-200"></div>
                          <div className="text-left min-w-[80px]">
                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Wait Time</p>
                            <p className={`text-[11px] font-black ${isClosed ? 'text-emerald-600' : 'text-amber-600'}`}>
    ...
    try {
      if (type === 'TICKET' || type === 'REQUEST') {
        const ticketPayload = {
          ...
          status: 'Open',
          created_by: user.id // FIX: This is strictly set to the UUID so the fetch loop catches it instantly!
        };
