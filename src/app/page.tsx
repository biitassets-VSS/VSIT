...
export default function LoginPage() {
  ...
  const themes = {
    Admin: {
      glow: 'bg-[#FF8A00]',
      button: 'bg-[#FF8A00] hover:bg-[#E67C00]',
      shadow: 'shadow-[0_8px_20px_-6px_rgba(255,138,0,0.5)]',
      badgeText: 'text-[#FF8A00]',
      badgeBg: 'bg-[#FFF4EA] border-[#FFE4C4]',
      focusRing: 'focus:border-[#FF8A00] focus:ring-[#FF8A00]/10',
      activeTab: 'text-[#FF8A00]'
    },
    Staff: {
      glow: 'bg-[#8B5CF6]', // Vibrant Purple
      ...
      focusRing: 'focus:border-[#8B5CF6] focus:ring-[#8B5CF6]/10',
      activeTab: 'text-[#8B5CF6]'
    },
    Guest: {
      glow: 'bg-[#10B981]', // Emerald Green
      button: 'bg-[#10B981] hover:bg-[#059669]',
      shadow: 'shadow-[0_8px_20px_-6px_rgba(16,185,129,0.5)]',
      badgeText: 'text-[#10B981]',
      badgeBg: 'bg-[#ECFDF5] border-[#D1FAE5]',
      focusRing: 'focus:border-[#10B981] focus:ring-[#10B981]/10',
      activeTab: 'text-[#10B981]'
    }
  };
  ...
  const handleTabChange = (tab: 'Admin' | 'Staff' | 'Guest') => {
    ...
    // Auto-fill for Guest, clear for others
    if (tab === 'Guest') {
      setEmail('guest@vss.com');
      setPassword('vss@123');
    } else {
      setEmail('');
      setPassword('');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Guaranteed Guest Fallback Logic
      // If using the default guest credentials, instantly bypass to dashboard 
      // (This prevents Supabase errors if the guest isn't in your Auth vault yet)
      ...
      // 2. Standard Admin & Staff Supabase Authentication
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#F4F7FB] font-sans relative overflow-hidden px-4 transition-colors duration-500">
      ...
      {/* Central Login Card with Dynamic Neon Glow */}
        ...
        <div className="bg-white rounded-[2.5rem] p-8 md:p-10 shadow-2xl shadow-slate-200/50 relative overflow-hidden">
          ...
          <div className="text-center mb-8 relative z-10">
            <img 
              ...
              className="h-16 mx-auto mb-4 object-contain transition-transform hover:scale-105 duration-300"
              ...
              return (
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-all duration-300 ${
                    isActive 
                      ? `bg-white shadow-sm border border-slate-100 ${themes[tab].activeTab}` 
          ...
          <form onSubmit={handleLogin} className="space-y-5 relative z-10">
            <div>
              <label className="block text-xs font-bold text-[#4A5568] mb-2 transition-colors">
                Email Address
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={activeTab === 'Admin' ? 'admin@virtualstaffing.com' : 'employee@virtualstaffing.com'}
                  className={`w-full pl-11 pr-4 py-3.5 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-800 outline-none transition-all focus:bg-[#F0F4FA] ${currentTheme.focusRing}`}
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••••••"
                  className={`w-full pl-11 pr-4 py-3.5 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-800 outline-none transition-all ${currentTheme.focusRing}`}
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full mt-2 py-4 rounded-xl text-sm font-black text-white transition-all duration-500 flex items-center justify-center gap-2 ${currentTheme.button} ${currentTheme.shadow}`}
            >
      ...
      <div className="mt-8 text-xs font-medium text-slate-500 z-10 relative">
        Design by <span className={`font-bold transition-colors duration-500 ${currentTheme.activeTab}`}>Ainodeat</span>
      </div>
    </div>
  );
}