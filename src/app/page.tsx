...
export default function StaffDashboardPage() {
  ...
  return (
    <>
      ...
      `}} />
      <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-8 font-inter text-slate-800 antialiased selection:bg-blue-200">
...
function LiveDatabaseModal({ type, asset, user, onClose }: any) {
  ...
  const handleLivePostgresSubmit = async () => {
    ...
    try {
      if (type === 'TICKET' || type === 'REQUEST') {
        // 🚨 FIX: WE NOW PASS `emp_code` TO AVOID THE DATABASE NOT-NULL CRASH!
        const ticketPayload = {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in text-left font-inter">
